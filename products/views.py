...
from .models import Product, ProductFile,ProductComments,ProductFAQ,ProductSeller,ProductCategories
from django.conf import settings
from .forms import ProductForm,ProductSellerForm
...
def product_add_existing(request):
    if request.method == 'POST':
        form = ProductSellerForm(request.POST, request.FILES)
        if form.is_valid():
...
def product_detail_view(request, pk=None, *args, **kwargs):
    ...
    print(product_obj)
    sellers = ProductSeller.objects.filter(product=instance)
    comments =ProductComments.objects.filter(product=instance)
